...
def create_EV_landscape(ev_array, condition, output_dir, optimal_input):
...
def get_EV_array(standard_deviation, condition, graph_input, optimal_input, LA_score):
...
def graph_by_val_sep_test_within(test_frame, optimal_dict, output_dir):
    ...
    for sep in abs_seps:
        for pen in pen_vals:
            ...
            avg_mark = plt.Circle((avg_x, avg_y), size = 1,color='b',fill=False)
            target   = plt.Circle((0,0), size = RADIUS,color='g',fill=False)
