...
CV_SUFFIX   = "_condition_variances.csv"

#grid of each aim point for simulation
XY_GRID   = expand.grid(c(-100:100), c(-100:100)) #c(-50, -40, -30, -20, -10, 0, 10, 20, 30, 40, 50), c(-50, -40, -30, -20, -10, 0, 10, 20, 30, 40, 50)
...
# model    = readline(prompt = "Include loss aversion score (Y/N): ")

wd = getwd()
# input_dir   = paste(wd, "/standard_data/", sub_string, sep = "", collapse = "")
# output_dir  = paste(wd, "/model_output/",  sub_string, '_', model, sep = "", collapse = "")
...
#MAIN LOOP
for (sub_int in c(500,  501, 502, 503, 504, 505, 506)){ #403, 404, 405, 406, 407, 410, 411, 412 
    for (model in c('lcv', 'nlcv')){
        ...
        sub_test_file   = paste(input_dir, SD_SUFFIX,   sep = "", collapse = "")
        test_data_frame = read.csv(sub_test_file)
        cvar_file   = paste(input_dir, CV_SUFFIX, sep = "", collapse = "")
        cvar_frame  = read.csv(cvar_file)
        ...
        print("Old Loss aversion score")
        print("-------------------")
        print(loss_aversion)

        if (loss_aversion < 1 | loss_aversion > 20) {
            loss_aversion = 1
        }

        print("New Loss aversion score")
        print("-------------------")
        print(loss_aversion)
        ...
        for (sep in SEP_DISTS){
            ...
            for (pen in PEN_VALS){
                abs_pen = -pen
                cvar_col = paste('X', sep, '.', abs_pen, sep = "", collapse = "")
                cvar = mean(cvar_frame[[cvar_col]])
                csd  = sqrt(cvar)
                print("variability")
                print("-----------")
                print(csd)
                ...
                for (n in c(1:nrow(XY_GRID))){
                    ...
                    temp_ev   = getExpectedValue(temp_aim, SIM_SIZE, csd, pen, sep, loss_aversion)
                    graph_EVs = c(graph_EVs, temp_ev)
                    

                }
                writeGraphData(graph_EVs, pen, sep, loss_aversion, csd)
            }
        }
    }   
}    